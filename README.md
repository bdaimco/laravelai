...
The production job publishes the tested archive as a `production-<commit-sha>` GitHub Actions artifact and a GitHub Release with the same commit-based tag on pushes to `main` and on manual `deploy-production` dispatches. It does not require production SSH credentials; configure approval rules on the `production` Environment if publishing needs review. The workflow needs `contents: write` permission to create releases.